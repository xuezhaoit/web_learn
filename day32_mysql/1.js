...
(async()=>{
  ...
  // fs.writeFile('a.txt', question_sql, (err) => {
  //   if (err) throw err;
  ...
  for(let ID in answers){
    ...
    aAnswers.push(dataJoin(ID, answer.question_ID,  answer.content, answer.createdTime,answer.author_ID));
  }
  let answer_sql=`INSERT INTO answers_table VALUES${aAnswers.join(',')}`;
  fs.writeFile('a.txt', answer_sql, (err) => {
    if (err) throw err;
    console.log('It\'s saved!');
  });
  //topic_sql
